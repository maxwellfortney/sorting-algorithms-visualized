  ...
  switch (randomThemeindex) {
    case 0:
      document.getElementById("favicon").href =
        "https://drive.google.com/uc?export=view&id=1Rb9JdoAoiDBMIXp4gNWqsKceDNAhw67M";
      break;
    case 1:
      document.getElementById("favicon").href =
        "https://drive.google.com/uc?export=view&id=1qjVeP7EhH07ZFIAEiW8HMdOflHqVGcbO";
      break;
    case 2:
      document.getElementById("favicon").href =
        "https://drive.google.com/uc?export=view&id=1e3Jgg7RItgIC5wkaxmeHaLbQOwR113TG";
      break;
  }
...
class App extends React.Component {
  ...
  updateSortConfig(sortConfig, newArray = false) {
    ...
    if (newArray) {
      ...
      this.setState({
        sortConfig,
        barsArray: generateBarsArray(sortConfig.arraySize),
      });
    }
  }
  ...
  calcMergeAnimationSpeed() {
    if (50 >= this.state.barsArray.length && this.state.barsArray.length > 0) {
      ...
      100 >= this.state.barsArray.length &&
      this.state.barsArray.length > 50
    ) {
      return 10;
    } else if (
      300 >= this.state.barsArray.length &&
      this.state.barsArray.length > 100
    ) {
      return 6;
    }
  }
  ...
  async mergeSort() {
    ...
    for (let i = 0; i < animations.length; i++) {
      ...
      if (isColorChange) {
        ...
        const color =
          i % 3 === 0
            ? this.state.theme.comparisonBarColor
            : this.state.theme.mainBarColor;
        setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          if (!this.state.isMuted) {
            try {
              synth.triggerAttackRelease(newHeight, 0.1);
            } catch (e) {
              // Nothing
            }
          }
  ...
  async heapSort() {
    ...
    for (let i = 0; i < animations.length; i++) {
      ...
      if (
        ...
        animations[i].type === "compare-remove"
      ) {
        ...
        setTimeout(() => {
          ...
          if (!this.state.isMuted) {
            try {
              synth.triggerAttackRelease(barOneHeight, 0.1);
              synth.triggerAttackRelease(barTwoHeight, 0.1);
            } catch (e) {
              // Nothing
            }
          }
  ...
  async quickSort() {
    ...
    for (let i = 0; i < animations.length; i++) {
      ...
      if (
        ...
        animations[i].type === "set-pivot" ||
        animations[i].type === "remove-pivot"
      ) {
      ...
      <div id="App" style={{ backgroundColor: theme.backgroundColor }}>
        {!this.state.isMuted ? (
          <svg
            ...
            viewBox="0 0 75 75"
            fill={theme.navTextColor}
            height="18px"
            style={{ position: "absolute", top: "7px", right: "35px" }}
          >
            <polygon
              stroke={theme.navTextColor}
              stroke-width="5"
              stroke-linejoin="round"
              points="39.389,13.769 22.235,28.606 6,28.606 6,47.698 21.989,47.698 39.389,62.75 "
            />
            <path
              fill="none"
              stroke={theme.navTextColor}
        ...
        <svg
          ...
          onClick={() => this.setState({ theme: generateTheme() })}
          viewBox="0 0 512 512"
          height="15px"
          fill={theme.navTextColor}
        ...
        <SettingsSelectors
          ...
          updateIsSorting={(x) => this.setState({ isSorting: x })}
          updateSortConfig={(sortConfig, newArray) =>
            this.updateSortConfig(sortConfig, newArray)
          }
